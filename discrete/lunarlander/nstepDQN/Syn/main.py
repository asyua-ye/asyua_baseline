...
class Hyperparameters:
    ...
    # nstep
    nstep: int = 3  # -1表示从回合开始累计奖励到回合结束,正常是大于0的整数
...
def train_online(RL_agent, env, eval_env, rollout, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-nstepDQN-{hp.total_epoches}')
    ...
    for t in range(int(hp.total_epoches+1)):
        ...
        states,actions,next_states,rewards,masks,exps = rollout.preTrj()
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: nstepDQN, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = nstepDQN.agent(state_dim, action_dim, hp)
    
    rollout = RolloutBuffer(hp.num_steps, hp.num_processes, state_dim, hp.discount, hp.nstep, hp.reward_scale_enable)
