...
from buffer import RolloutBuffer,PrioritizedReplayBuffer,priorReplayBuffer
...
def train_online(RL_agent, env, eval_env, rollout, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-prioDQN-{hp.total_epoches}')
    ...
    for t in range(int(hp.total_epoches+1)):
        ...
        for data in replaybuffer.sample():
            weight,idxs = RL_agent.train(data,process,writer)
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: prioDQN, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = prioDQN.agent(state_dim, action_dim, hp)
    
    rollout = RolloutBuffer(hp.num_steps, hp.num_processes, state_dim, hp.discount, hp.reward_scale_enable)
    replaybuffer = priorReplayBuffer(state_dim,hp)
