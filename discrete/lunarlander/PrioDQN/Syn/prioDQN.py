...
class agent(object):
    ...
    def train(self,data,process,writer):
        ...
        weight = (q_values - target_q_values).sum(-1).abs().detach()
        loss_element_wise = F.mse_loss(q_values, target_q_values, reduction='none')
        loss = (is_weight * loss_element_wise).mean()
