...
class agent(object):
    ...
    @torch.no_grad()   
    def projection_distribution(self,dist,next_state,reward,mask,exp):
        ...
        Tz = reward + (self.gamma ** exp) * next_dist * mask
    ...
    def train(self,data,process,writer):
        ...
        dist = all_p.gather(1, actions.long().unsqueeze(1).expand(all_p.size(0), 1, all_p.size(2))).squeeze(1)
        tau,next_dist = self.projection_distribution(dist,next_states,rewards,masks,exps)
        
        diff = next_dist.t().unsqueeze(-1) - dist.unsqueeze(0)
        huber_loss = Huber_loss(diff)
        loss = ((tau - (diff < 0).float()).abs() * huber_loss)
        loss = loss.transpose(0,1)
        loss_sum = loss.mean(1).sum(-1,keepdim=True)
