...
def worker(remote, parent_remote, env_fn_wrappers,env_name):
...
class SubprocVecEnv(VecEnv):
    ...
    """
    def __init__(self, env_fns, env_name, spaces=None, context='spawn',in_series=1):
        """
        Arguments:
        ...
        in_ser = [in_series for _ in range(self.nremotes)]
        self.ps = [ctx.Process(target=worker, args=(work_remote, remote, in_serie, env_name))
                   for (work_remote, remote, in_serie) in zip(self.work_remotes, self.remotes, in_ser)]
        for p in self.ps:
...
def _flatten_obs(obs):
    ...
    if isinstance(obs[0], dict):
        ...
        """
        每个线程的局势不一样，而状态有两种类型（值,dict）  (值)
        """
        temp=[]
        for i in range(len(obs)):
            if isinstance(obs[i][1],dict):
                temp.append(obs[i][0])
            else:
                temp.append(obs[i])
        a=np.stack(temp)
