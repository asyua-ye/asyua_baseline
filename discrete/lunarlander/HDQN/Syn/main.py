...
class Hyperparameters:
    ...
    state_norm_enable: bool = True
    reward_scale_enable: bool = True 
    eps: float = 1e-5
...
def train_online(RL_agent, env, eval_env, rollout, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-HDQN-{hp.total_epoches}')
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: HDQN, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = HDQN.agent(state_dim, action_dim, hp)
    
    rollout = RolloutBuffer(hp.num_steps, hp.num_processes, state_dim, hp.discount, hp.reward_scale_enable)
    replaybuffer = ReplayBuffer(hp.buffer_size,hp.num_processes,hp.num_steps,state_dim,hp.batch,hp.epoch_per_train)
