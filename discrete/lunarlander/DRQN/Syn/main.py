...
def train_online(RL_agent, env, eval_env, rollout, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-DRQN-{hp.total_epoches}')
    ...
    for t in range(int(hp.total_epoches+1)):
        ...
        for step in range(hp.num_steps):
            rounds += 1
            action,seq_state,(hidden, cell) = RL_agent.select_action(np.array(state),mask)
            ...
            rollout.insert(seq_state, action, reward, mask, hidden, cell)
            state = next_state
            mask = next_mask
        ...
        s = time.time()
        next_state = RL_agent.processState(next_state,next_mask)
        rollout.lastInsert(next_state.to(hp.device),next_mask)
        states,actions,next_states,rewards,masks = rollout.preTrj()
        data=(np.copy(states), np.copy(actions),np.copy(next_states), np.copy(rewards),np.copy(masks))
...
def maybe_evaluate_and_print(RL_agent, eval_env, t, start_time, hp):
    ...
    if hp.checkpoint or hp.test:
        ...
        if hp.num_processes == 1:
            ...
            state,mask = eval_env.reset(),torch.ones((hp.num_processes,1)).to(hp.device)
            
        episode_rewards = np.zeros(hp.num_processes, dtype=np.float64)
        final_rewards = np.zeros(hp.num_processes, dtype=np.float64)
        fin = 0
        while True:
            action,_,_ = RL_agent.select_action(np.array(state),mask,True)
            if hp.num_processes != 1:
            ...
            mask = torch.from_numpy(mask).view(-1, 1).to(hp.device)
            if np.any(done==1):
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: DRQN, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    hp.max_steps = hp.max_epoches * hp.epoch_per_train
    ...
    RL_agent = DRQN.agent(state_dim, action_dim, hp)
