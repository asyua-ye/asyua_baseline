...
def train_online(RL_agent, env, eval_env, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-EDAC-{hp.total_epoches}')
    train = []
...
def train_offline(RL_agent, eval_env, replaybuffer, hp):
    ...
    for t in range(int(args.total_epoches)):
        s0 = time.time()
        if t % hp.eval_freq == 0:
            s = time.time()
...
if __name__ == "__main__":
    ...
    # RL
    parser.add_argument("--env", default="Walker2d-v4", type=str)
    parser.add_argument("--offlineEnv", default="walker2d-medium-v2", type=str)
    ...
    print("---------------------------------------")
    print(f"Algorithm: EDAC, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = EDAC.agent(state_dim, action_dim, max_action, hp)
    ...
    if args.test:
        ...
        dataset_path = f'E:/python/rl-work/GitHub/asyua_baseline/offline/dataset/{args.offlineEnv}.hdf5'
