...
def train_online(RL_agent, env, eval_env, rollout, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-DDPG-{hp.total_epoches}')
    ...
    for t in range(int(hp.total_epoches+1)):
        ...
        for step in range(hp.num_steps):
            ...
            state = torch.from_numpy(state.astype(np.float32)).to(hp.device)
            action = torch.from_numpy(action.astype(np.float32)).to(hp.device)
        ...
        if hp.checkpoint:
            ...
            if rs!=0:
                ...
                all_score = np.mean(total_reward) + np.mean(total_eval_reward)
                sample_score = np.mean(total_reward)
            else:
                all_fin = 0
                all_score = 0.0
        ...
        if t > hp.begin_time:
            ...
            e = time.time()
            train_time = (e-s1)   
            if hp.checkpoint and t%10==0:
                s = time.time()
                text,total_eval_reward,eval_fin = maybe_evaluate_and_print(RL_agent, eval_env, train_num, start_time, hp)
                e = time.time()
                eval_time = (e-s)
                train.extend(text)
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: DDPG, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = DDPG.agent(state_dim, action_dim, max_action, hp)
    
    rollout = RolloutBuffer(state_dim, action_dim, hp)
