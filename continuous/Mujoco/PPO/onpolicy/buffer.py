...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim, hp):
        ...
        # 预分配 NumPy 数组
        self.states = np.zeros((self.max, self.numactor * self.rollout,state_dim), dtype=np.float32)
        self.actions = np.zeros((self.max, self.numactor * self.rollout,action_dim), dtype=np.float32)
        self.next_states = np.zeros((self.max, self.numactor * self.rollout,state_dim), dtype=np.float32)
