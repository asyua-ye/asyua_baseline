...
class Hyperparameters:
    ...
    # SO2
    N_upc: int = 10    # 交互一步训练N_upc次？应该是这意思
    noise_std: float = 0.3
    c: float = 0.6
...
def train_online(RL_agent, env, eval_env, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-SO2-{hp.total_epoches}')
    train = []
...
def train_offlineTOonline(RL_agent, env, eval_env, replaybuffer, writer, process, train, start_time, file_time, hp):
    ...
    train_num = 0
    state,_ = env.reset()
    replaybuffer.train_per_epoch = hp.N_upc
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: SO2, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = SO2.agent(state_dim, action_dim, max_action, hp)
