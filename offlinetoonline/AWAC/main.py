...
def train_online(RL_agent, env, eval_env, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-AWAC-{hp.total_epoches}')
    train = []
...
def train_offlineTOonline(RL_agent, env, eval_env, replaybuffer, writer, process, train, start_time, file_time, hp):
    total_eval_reward = np.zeros(hp.eval_eps)
    eval_time = 0
    ep_total_reward = 0
    ep_num = 0
...
def train_offline(RL_agent, env, eval_env, replaybuffer, hp):
    start_time = time.time()
    file_time = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-AWAC-{hp.total_epoches}')
    train = []
    ...
    if hp.offlinetoonline:
        train_offlineTOonline(RL_agent, env, eval_env, replaybuffer, writer, process, train, start_time, file_time, hp)
    eval_env.close()
    file_time1 = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    log_and_print(train, f"\nend time: {file_time1}")
    np.savetxt(f"./output/{hp.file_name}/{file_time}/train.txt", train, fmt='%s')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_epoches", default=25, type=int)
    ...
    print("---------------------------------------")
    print(f"Algorithm: AWAC, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = AWAC.agent(state_dim, action_dim, max_action, hp)
    ...
    if args.test:
        ...
        replaybuffer.load_dataset(dataset_path)
        train_offline(RL_agent, env, eval_env,replaybuffer,hp)
    else:
        from torch.utils.tensorboard import SummaryWriter
        if args.retrain:
            file_name = f"./test/{args.file_name}/model/"
