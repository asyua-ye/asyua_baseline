...
import time
import datetime
import offline2online
import torch.nn.functional as F
from buffer import ReplayBuffer,priorReplayBuffer
...
class Hyperparameters:
    ...
    eps: float = 1e-5
    batch: int = 256
    epoch_per_train: int = 1000
    train_per_epoch: int = 1000
    allowTrue: int = 5e3
    eval_freq: int = 1
    online_eval_freq: int = 10e3
    OTO_epoches: int = 25e4
...
def train_online(RL_agent, env, eval_env, replaybuffer, hp):
    ...
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-offline2online-{hp.total_epoches}')
    train = []
...
def train_offlineTOonline(RL_agent, env, eval_env, offlineReplaybuffer,replaybuffer,prioReplaybuffer, writer, process, train, start_time, file_time, hp):
    total_eval_reward = np.zeros(hp.eval_eps)
    eval_time = 0
    ep_total_reward = 0
    ep_num = 0
    ...
    for t in range(int(hp.OTO_epoches)):
        ...
        mask = 1.0 - float(ep_finished) 
        replaybuffer.push(state, action, next_state, reward, mask)
        prioReplaybuffer.push(state, action, next_state, reward, mask)
        state = next_state
        ...
        if t % hp.epoch_per_train == 0 and t > hp.allowTrue:
            ...
            s1 = time.time()
            RL_agent.training(True)
            for data in prioReplaybuffer.sample():
                sample,ind = data
                offline_sample = offlineReplaybuffer.one_sample()
                online_sample = replaybuffer.one_sample()
                weights = RL_agent.train(offline_sample,online_sample,sample,process,writer)
                prioReplaybuffer.update_batch(ind,weights)
                
            RL_agent.training(False)
            e = time.time()
            train_time = (e-s1)   
            
            if t % hp.online_eval_freq == 0:
                s = time.time()
                text,total_eval_reward = maybe_evaluate_and_print(RL_agent, eval_env, train_num, start_time, hp)
                e = time.time()
                eval_time = (e-s)
...
def train_offline(RL_agent, env, eval_env, offlineReplaybuffer,replaybuffer,prioReplaybuffer, hp):
    start_time = time.time()
    file_time = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    writer = SummaryWriter(f'./output/{hp.file_name}/{file_time}/{file_time}-offline2online-{hp.total_epoches}')
    train = []
    ...
    for t in range(int(hp.total_epoches)):
        ...
        if t % hp.eval_freq == 0:
            s = time.time()
            text,total_eval_reward = maybe_evaluate_and_print(RL_agent, eval_env, t, start_time, hp)
            e = time.time()
            eval_time = (e-s)
            train.extend(text)
            writer.add_scalar('eval_reward', np.mean(total_eval_reward), global_step=t)
    ...
    if hp.offlinetoonline:
        train_offlineTOonline(RL_agent, env, eval_env, offlineReplaybuffer, replaybuffer, prioReplaybuffer, writer, process, train, start_time, file_time, hp)
    eval_env.close()
    file_time1 = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    log_and_print(train, f"\nend time: {file_time1}")
    np.savetxt(f"./output/{hp.file_name}/{file_time}/train.txt", train, fmt='%s')
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    print(f"Algorithm: offline2online, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    RL_agent = offline2online.agent(state_dim, action_dim, max_action, hp)
    
    offlineReplaybuffer = ReplayBuffer(state_dim, action_dim, hp)
    replaybuffer = ReplayBuffer(state_dim, action_dim, hp)
    prioReplaybuffer = priorReplayBuffer(state_dim, action_dim, hp)
    ...
    if args.test:
        ...
        dataset_path = f'E:/python/rl-work/GitHub/asyua_baseline/offlinetoonline/dataset/{args.offlineEnv}.hdf5'
        offlineReplaybuffer.load_dataset(dataset_path)
        prioReplaybuffer.load_dataset(dataset_path)
        train_offline(RL_agent, env, eval_env,offlineReplaybuffer,replaybuffer,prioReplaybuffer,hp)
    else:
        from torch.utils.tensorboard import SummaryWriter
        if args.retrain:
            file_name = f"./test/{args.file_name}/model/"
